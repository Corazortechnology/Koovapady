...
const FamilySection = () => {
  ...
  // choose image: prefer an image with label === 'thumbnail' or whose public_id/url includes 'thumbnail'
  const pickThumbnailImage = (folder) => {
    if (!folder || !Array.isArray(folder.images) || folder.images.length === 0) return null;

    // find by explicit label === 'thumbnail'
    const byLabel = folder.images.find((img) => img && img.label && img.label.toLowerCase() === "thumbnail");
    if (byLabel && typeof byLabel.url === "string" && byLabel.url.trim()) return byLabel.url;

    // fallback: find image whose public_id or url looks like a thumbnail
    const byPublicId = folder.images.find((img) => {
      if (!img) return false;
      const pid = (img.public_id || "").toString().toLowerCase();
      const url = (img.url || "").toString().toLowerCase();
      return pid.includes("thumbnail") || url.includes("thumbnail");
    });
    if (byPublicId && typeof byPublicId.url === "string" && byPublicId.url.trim()) return byPublicId.url;

    // final fallback: first image with a valid url
    const first = folder.images.find((img) => img && typeof img.url === "string" && img.url.trim());
    ...
    const fetchFolders = async () => {
      ...
      try {
        ...
        const mapped = (Array.isArray(data) ? data : []).map((f, i) => {
          ...
          const subtitle = f.name || "Folder";
          const year = f.createdAt ? new Date(f.createdAt).getFullYear() : "";
          const image = pickThumbnailImage(f) || PLACEHOLDER_IMAGE;
          const align = i % 2 === 0 ? "left" : "right";
          const link = `/folder/${id}`;
